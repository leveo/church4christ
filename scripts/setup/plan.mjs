...
export function buildSetupPlan(answers, catalog, currentState = {}) {
  ...
  const existingMode = currentState.mode ?? currentState.existingMode;
  if (currentState.existingBackend && existingMode && existingMode !== normalized.mode) {
    throw new Error(
      `Existing ${existingMode}-to-${normalized.mode} mode migration is not implemented; ` +
      'use an explicit export/import workflow or a fresh installation',
    );
  }
