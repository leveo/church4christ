...
const PROVIDER_CRONS = Object.freeze({
  d1: Object.freeze(['0 13 * * *', '0 14 * * 4', '0 9 * * *']),
  supabase: Object.freeze(['0 13 * * *', '0 14 * * 4', '*/5 * * * *']),
});
const WORKER_CRONS = Object.freeze(['0 13 * * *', '0 14 * * 4', '0 9 * * *', '*/5 * * * *']);
...
export async function checkConfig(options) {
  ...
  if (!Array.isArray(configuredCrons) || configuredCrons.some((value) => typeof value !== 'string') ||
    ...
    checks.push(result('config.worker-crons', 'error', 'Scheduled Worker source and provider-specific cron triggers do not match the canonical schedules.', 'Restore reminder, digest, D1 backup, and Supabase Stripe recovery schedules.'));
  }
