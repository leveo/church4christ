...
const PROVIDER_CRONS = Object.freeze({
  d1: Object.freeze(['0 13 * * *', '0 14 * * 4', '0 9 * * *', '0 * * * *', '15,45 * * * *']),
  supabase: Object.freeze(['0 13 * * *', '0 14 * * 4', '0 * * * *', '15,45 * * * *', '*/5 * * * *']),
});
const WORKER_CRONS = Object.freeze([
  '0 13 * * *', '0 14 * * 4', '0 * * * *', '15,45 * * * *', '0 9 * * *', '*/5 * * * *',
]);
...
export async function checkConfig(options) {
  ...
  const stripeModeOk = options.manifest.database === 'supabase'
    ? parsed.vars?.STRIPE_MODE === 'test'
    : !Object.hasOwn(parsed.vars ?? {}, 'STRIPE_MODE');
  if (!stripeModeOk) {
    checks.push(result('config.stripe-mode', 'error', 'STRIPE_MODE must be test for Supabase and absent for D1.', 'Rerun setup to restore the provider-specific test-mode marker.'));
  }
  let desired;
  ...
  const expectedConfigured = PROVIDER_CRONS[options.manifest.database];
  if (!Array.isArray(configuredCrons) || configuredCrons.some((value) => typeof value !== 'string') ||
      !same(configuredCrons, expectedConfigured) || !sourceCrons || !same(sourceCrons, WORKER_CRONS)) {
    checks.push(result('config.worker-crons', 'error', 'Scheduled Worker source and provider-specific cron triggers do not match the canonical schedules.', 'Restore reminder, digest, group attendance, D1 backup, and Supabase Stripe recovery schedules.'));
  }
