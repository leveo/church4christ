...
/**
 * Per-theme metadata for the admin theme picker: the label, its default color
 * mode, and three representative swatches (primary / accent / surface) read from
 * that theme's DEFAULT-mode palette. Pure — `themes` is the parsed theme JSONs.
 */
export function generateThemeMeta(themes) {
  return themes.map((theme) => {
    const palette = theme.modes[theme.defaultMode];
    return {
      name: theme.name,
      label: theme.label,
...
/** Serialize the theme-meta array into the generated TS module source. */
export function themeMetaModule(meta) {
  return (
    '// AUTO-GENERATED by scripts/build-tokens.mjs — do not edit.\n' +
    '// Source of truth: design/themes/*.json (each theme\'s default-mode swatches).\n' +
    `export const THEME_META = ${JSON.stringify(meta, null, 2)} as const;\n`
  );
}

/** Write src/lib/themeMeta.generated.ts under `root`; returns the output path. */
export function writeThemeMeta(root, themes) {
  const out = join(root, 'src/lib/themeMeta.generated.ts');
  mkdirSync(dirname(out), { recursive: true });
  writeFileSync(out, themeMetaModule(generateThemeMeta(themes)));
  return out;
}
...
function main() {
  ...
  writeThemeMeta(root, themes);
  console.log(
    `tokens: wrote src/styles/tokens.generated.css + src/lib/themeMeta.generated.ts ` +
