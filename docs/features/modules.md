...
- **Service Attendance** works without Serve and keeps using existing service types. Turn
...
The Full Church preset enables all 18 modules; Website enables 8; Website + Community
enables all 15 modules that are compatible with D1. Custom setup can select individual
modules.
...
- **Registry:** `config/capabilities.json` is the canonical, validated catalog, adapted by
  `src/lib/modules.ts` for runtime use. It maps each of the 18 keys to the locale-stripped
...
- **Enablement + cache:** new setup writes all 18 `module.<key>` rows explicitly. For legacy
...
- **Admin panel:** `src/pages/admin/settings/index.astro` renders the grouped checkboxes and
  writes all 18 `module.<key>` rows explicitly (an unchecked box is written as `'0'`, not
