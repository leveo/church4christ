...
The Full Church preset enables all 16 modules; Website enables 8; Website + Community
enables all 13 modules that are compatible with D1. Custom setup can select individual
modules.
...
- **Registry:** `config/capabilities.json` is the canonical, validated catalog, adapted by
  `src/lib/modules.ts` for runtime use. It maps each of the 16 keys to the locale-stripped
...
- **Enablement + cache:** new setup writes all 16 `module.<key>` rows explicitly. For legacy
