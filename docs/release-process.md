# v1.2.0 source release process

Church4Christ `v1.2.0` is a maintainer-created source checkpoint for operators who need a
reviewed revision and explicit community workflow upgrade notes. Version `v1.0.0` remains the historical
...
No release is created merely because `package.json` currently says `1.2.0`. The immutable
...
Version 1.2.0 adds forward files `0027` through `0038`, including optional fellowship and campus workflows.

## Prepare the release pull request

1. Confirm that the release target is exactly `v1.2.0` and that the 1.1.0 → 1.2.0 upgrade
   covers `0027_multi_campus.sql` through `0038_community_workflows.sql` on both backends.
   Keep the historical 1.0.0 baseline rehearsal as an additional forward-upgrade check.
...
2. create an annotated `v1.2.0` tag on that exact commit;
