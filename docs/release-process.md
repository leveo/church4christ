...
No release is created merely because `package.json` currently says `1.0.0`. The changelog
...
`0016` in both `migrations/` and `migrations-supabase/` are the frozen current `main` baseline.
