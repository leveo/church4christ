...
2. **Giving history** — owners see the whole household's gifts; other members
   see only their own.
3. **Groups = fellowships** — fellowships (团契) get a real membership model
   (upgraded from content-only pages to DB entities). Members see their
   fellowships and fellowships open for sign-up. Serving teams stay
   serving-only.
...
6. **Calendar** — one personal calendar combining serving, registered events,
   and fellowship meetings; subscribable from Google/Apple Calendar; blockout
   dates editable here and visible to serving admins.
7. **Prayer wall** — church-public, group-scoped, event-scoped, and private
   prayer items, with scoped approval (church admin / fellowship leader / event
...
- **Fellowships become DB entities** (owner decision). New `fellowships` +
  `fellowship_i18n` tables in **both** backends (public content must keep
  working on D1); the public `/[locale]/fellowships` page switches from
  content collections to DB via the standard `i18nJoin` pattern, and the admin
  console gains a fellowship editor (definitions, meeting schedule, leaders).
  The `src/content/fellowships/` collection is retired (existing sample
  content becomes seed data). Membership (`fellowship_members`,
  `fellowship_applications`) is **Supabase-only** and portal-gated — on D1 the
...
- **Prayer approval is scoped**: `church` scope → `people.role = 'admin'`;
  `group` scope → that fellowship's `fellowship_members.is_leader = 1`;
...
- **Google/Apple calendar link = ICS subscription.** Extend the existing
  ...
  events and fellowship meetings. The portal calendar page shows a `webcal://`
  ...
  UTC→wall-clock with the existing `dates.ts` helpers; fellowship-meeting
  occurrences are computed over a bounded window (past 30 days → +180 days)
  with stable per-occurrence UIDs (`fellowship-{id}-{yyyy-mm-dd}`). The feed
...
- **Shared-table columns land in both backends** (D1 + Postgres migrations)
  ...
  the `tokens` purpose-constraint rebuild, and the `fellowships` /
  `fellowship_i18n` tables. Portal-only tables (`event_admins`,
  `prayer_items`, `fellowship_members`, `fellowship_applications`) are
  Supabase-only, following the giving/registration precedent.

## Approaches considered

**A. Expand `/my` into the portal; supabase-only module; fellowships upgraded
to DB entities; scoped-approval prayer wall (chosen).** Maximum reuse: route
policy (`authed` class + fail-closed `/my` gate), magic-link auth, ICS feed,
month-calendar component, household / plan / giving data layers all exist. New
code concentrates on fellowship membership, household ownership, prayer items,
and read-model pages.
...
tokens rebuild, fellowships) and `migrations-supabase/0006_member_portal.sql`
...
CREATE TABLE fellowship_members (
  ...
  UNIQUE (fellowship_id, person_id)
);
...
CREATE TABLE prayer_items (
  ...
  fellowship_id INTEGER REFERENCES fellowships(id),   -- required iff scope=group
...
CREATE INDEX idx_prayer_items_fellowship ON prayer_items(fellowship_id);
...
| Post prayer item | member: church scope + own fellowships + events they're registered for + private |
| Approve church prayer | `role='admin'` |
| Approve group prayer | that fellowship's leader (`is_leader=1`); church admin |
| Approve event prayer | that event's `event_admins`; church admin |
| See group-scoped prayers | members of that fellowship |
| See event-scoped prayers | registered persons of that event + its event admins |
| Apply to join a fellowship | any member, when `open_signup=1` |
| Approve fellowship applications | that fellowship's leader; church admin |
| Set own blockout dates | self (existing) |

Enforcement lives in the data layer (`portalDb`/`prayerDb`/`fellowshipDb`
functions take the viewer's person id and scope every query), not in
templates. `SessionUser` already carries `isAdmin`; fellowship membership /
...
| `/my/groups` | My fellowships (with meeting schedule) + open fellowships with apply button; leaders see pending applications |
...
| `/my/calendar` (expand) | Month grid gains registered events + fellowship meetings alongside serving + blockouts (portal-gated in-page); blockout editing as today; "Subscribe in Google/Apple Calendar" panel with the `webcal://` feed link + regenerate token |
...
(`i18nJoin`), gains meeting time/location display; visual design unchanged.
...
3. **Fellowship editor (new)**: CRUD for fellowships + i18n names/descriptions,
   meeting weekday/time/frequency/location, open_signup toggle; member +
   leader management (portal-module-gated, Supabase-only data).
...
taken by prayer-sheet strings). Fellowship content localized via
`fellowship_i18n` joins. User-generated content (prayer bodies) is not
localized.
...
- Unit (Vitest): owner promotion/demotion rules (max 2, no self-demote, no
  ...
  (window + stable UIDs), fellowship application flow.
- E2E (`test:e2e:pg`, since portal is Supabase-only): sign-in → edit household
  member → apply to fellowship → leader approves → post group prayer → leader
  approves → appears in group tab; event admin approves event prayer;
  non-owner cannot see household giving.
- Schema parity tests updated (`D1_FILES` + tokens rebuild fold).
- Seed data (`seed/dev-seed.sql` + supabase seed): fellowships (ported from
  the retired content collection), owners on seeded households, fellowship
  members/leaders, a few prayer items in each scope/status, an event_admin.
...
2. **Fellowships**: tables + admin fellowship editor, public page switch to
   DB, content-collection retirement, `/my/groups` with application flow.
