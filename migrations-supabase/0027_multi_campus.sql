-- PostgreSQL/Supabase parity for the multi-campus shared-backend boundary.
...
SELECT setval(
  pg_get_serial_sequence('campuses', 'id'),
  GREATEST((SELECT MAX(id) FROM campuses), 1),
  true
);
...
CREATE OR REPLACE FUNCTION add_default_campus_membership()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
  ...
  ON CONFLICT (campus_id, person_id) DO NOTHING;
  RETURN NEW;
END;
$$;
...
FOR EACH ROW EXECUTE FUNCTION add_default_campus_membership();
...
-- Tables shared with the D1 schema.
...
-- Supabase-only Giving, Registration, Portal, and payment-operation tables.
ALTER TABLE funds ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE fund_i18n ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE gifts ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE recurring_gifts ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE reg_events ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE reg_event_i18n ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE reg_questions ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE reg_question_i18n ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE registrations ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE reg_answers ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE group_files ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE event_admins ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE prayer_items ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE group_reg_events ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE church_private.stripe_webhook_events ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;
ALTER TABLE church_private.stripe_checkout_requests ADD COLUMN campus_id INTEGER NOT NULL DEFAULT 1;

-- Common hot-path partitions. Remaining tables are normally reached through a
-- campus-scoped parent join and still carry their direct enforcement key.
...
CREATE INDEX idx_funds_campus ON funds (campus_id);
CREATE INDEX idx_gifts_campus ON gifts (campus_id);
CREATE INDEX idx_reg_events_campus ON reg_events (campus_id);
CREATE INDEX idx_registrations_campus ON registrations (campus_id);
