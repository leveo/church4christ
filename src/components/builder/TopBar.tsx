...
export default function TopBar(p: TopBarProps) {
  return (
    <div className="builder-topbar space-y-3 rounded-xl border border-border bg-surface-raised p-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex overflow-hidden rounded-md border border-border text-sm">
          {(['en', 'zh'] as const).map((l) => (
            <button key={l} type="button" aria-pressed={p.editLocale === l} onClick={() => p.onLocale(l)}
        ...
        <button type="button" className={btnSecondary} disabled={!p.canUndo} onClick={p.onUndo} aria-label={p.strings.undo} title={p.strings.undo}>↩</button>
        <button type="button" className={btnSecondary} disabled={!p.canRedo} onClick={p.onRedo} aria-label={p.strings.redo} title={p.strings.redo}>↪</button>
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" checked={p.published} onChange={(e) => p.onMeta({ published: e.target.checked })} /> {p.strings.published}
        </label>
        <span className="grow" />
        {p.error && <span role="alert" className="text-sm text-danger">{p.error}</span>}
        {!p.error && p.dirty && !p.saving && <span className="text-sm text-warn">{p.strings.unsaved}</span>}
        {!p.error && p.savedFlash && !p.dirty && <span role="status" className="text-sm text-success">{p.strings.saved}</span>}
