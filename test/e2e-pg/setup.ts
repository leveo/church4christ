...
try {
  ...
  // The Activity Score model is migration-owned configuration rather than demo
  // content. The per-file TRUNCATE removes it, so restore the exact migration
  // defaults instead of letting one file's saved model leak into the next.
  await sql.unsafe(`
