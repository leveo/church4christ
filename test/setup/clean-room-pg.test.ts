...
suite('clean-room Supabase setup', () => {
  it('creates, verifies, serves, and safely reruns Full Church in a disposable database', async () => {
    ...
    try {
      ...
      expect((await db<{ total: number; enabled: number }[]>`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE value='1')::int AS enabled FROM settings WHERE key LIKE 'module.%'`)[0]).toEqual({ total: 19, enabled: 19 });
