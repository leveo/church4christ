// Group files data layer (Task 4 of the Member Portal Phase 2). The download
// ROUTE (src/pages/[locale]/my/groups/[id]/files/[fileId].ts) needs the full
// request pipeline and is exercised in the pg e2e suite (Task 6); here we
// unit-test the lib exhaustively: the ext+MIME allowlist, size/empty caps, the
// random (NOT content-addressed) key shape, the download ACL matrix, soft-delete
// + best-effort R2 delete, list ordering/exclusion, and the RFC 5987
// content-disposition builder.
//
// group_files/group_members are Supabase-only (migrations-supabase/0006), with no
// D1 counterpart, so — exactly as test/groupDb.test.ts documents for group_members
// — we CREATE TABLE IF NOT EXISTS them here matching the PG DDL minus identity.
// The real PG DDL is covered by the pg e2e suite.
import { env } from 'cloudflare:test';
import { beforeEach, describe, expect, it } from 'vitest';
import type { AppDb } from '../src/lib/appDb';
import { addGroupMember, saveGroup, type GroupInput } from '../src/lib/groupDb';
import {
...
async function reset(): Promise<void> {
  ...
  groupId = await saveGroup(env.DB, null, baseInput);
}
beforeEach(reset);
...
describe('saveGroupFile — persistence + key shape', () => {
  it('stores the row, puts the bytes in R2, and uses a random group-scoped key', async () => {
    ...
    expect(row).toEqual({
      group_id: groupId,
      uploaded_by: 2,
      file_name: 'Plan 2026.pdf', // ORIGINAL name preserved (RFC 5987 download handles unicode)
      r2_key: row!.r2_key,
...
describe('getGroupFileForDownload — ACL matrix', () => {
  ...
  beforeEach(async () => {
    ...
    await addGroupMember(db, groupId, 1); // person 1 is a member
  });
  ...
  it('mismatched groupId in the URL: returns null even for a member', async () => {
    const otherGroup = await saveGroup(db, null, { ...baseInput, slug: 'other' });
    expect(await getGroupFileForDownload(db, { fileId, groupId: otherGroup, personId: 1, isAdmin: false })).toBeNull();
    // even an admin can't cross a wrong group id to a file that isn't in it
...
describe('deleteGroupFile', () => {
  it('soft-deletes the row and deletes the R2 object; returns true', async () => {
    ...
    expect(await media.get(key)).toBeNull(); // best-effort R2 delete happened
  });
  ...
  it('does not throw when the R2 delete fails (best-effort)', async () => {
    ...
    const row = await db.prepare('SELECT deleted_at FROM group_files WHERE id = ?').bind(fileId).first<{ deleted_at: string | null }>();
    expect(row!.deleted_at).not.toBeNull(); // still soft-deleted
  });
});
...
describe('listGroupFiles', () => {
  it('excludes soft-deleted files, orders newest-first, and joins the uploader name', async () => {
    ...
    expect(rows.map((r) => r.id)).toEqual([c, a]); // newest first, soft-deleted excluded
  ...
  // NOTE: the COALESCE(display_name, first||' '||last) uploader-name fallback
  // mirrors groupDb.listGroupMembers but isn't unit-tested here — the D1 test
  // schema enforces people.display_name NOT NULL, so the null branch is
  // unreachable in this harness (same reason groupDb.test.ts omits it).
});
...
describe('contentDispositionAttachment', () => {
  ...
  it('percent-encodes a unicode name and strips non-ASCII from the fallback', () => {
    ...
    expect(out).toContain('filename=".pdf"'); // CJK stripped from the fallback
  });
