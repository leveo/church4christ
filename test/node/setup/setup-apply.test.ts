...
describe('setup secrets', () => {
  ...
  it('updates the managed local bypass identity when the setup administrator changes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'c4c-admin-bypass-')); const path = join(root, '.dev.vars');
    const session = 'existing-session-secret-with-at-least-32-chars';
    await writeFile(path, `OTHER=preserved\nSESSION_SECRET=${session}\nEMAIL_DEV_LOG=1\nAUTH_DEV_BYPASS_EMAIL=old@example.test\n`, { mode: 0o600 });

    await configureSecrets({ mode: 'local', adminEmail: 'New@Example.Test', path });

    const text = await readFile(path, 'utf8');
    expect(text).toContain('OTHER=preserved');
    expect(text).toContain(`SESSION_SECRET=${session}`);
    expect(text).toContain('AUTH_DEV_BYPASS_EMAIL=new@example.test');
    expect(text).not.toContain('AUTH_DEV_BYPASS_EMAIL=old@example.test');
  });
