...
describe('portable People export documentation', () => {
  ...
  it('records migration 0011 as the export forward step and 0012 as the frozen baseline', () => {
    expect(people).toMatch(/migration[^\n]*0011_people_exports\.sql/i);
    expect(upgrade).toMatch(/Files `0001` through `0012`/);
    expect(release).toMatch(/Files `0001` through\s+`0012`/);
    expect(changelog).toMatch(/Migration files `0001` through `0012`/);
