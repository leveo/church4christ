...
describe('shared media setup', () => {
  ...
  it('includes validated portal file objects only when the portal capability is selected', async () => {
    const plan = loadMediaPlan({ root: process.cwd(), includePortalFiles: true });
    ...
    expect(plan.objects[0].contentBase64).toBeTypeOf('string');
    expect(plan.objects[0]).not.toHaveProperty('filePath');

    const db = fakeDb();
    const upload = vi.fn(async () => undefined);
    await applyMediaPlan({ mediaPlan: plan, db: db.db, uploadObject: upload });
    expect(upload).toHaveBeenCalledWith(expect.objectContaining({
      key: 'group-files/1/demo-young-adults-welcome.pdf',
      contentType: 'application/pdf',
    }));
    expect(db.writes).toHaveLength(plan.assets.length * 2);
  });
