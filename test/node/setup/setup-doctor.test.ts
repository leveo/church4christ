...
import { ALWAYS_REQUIRED_TABLES, PRIVATE_TABLES_BY_CAPABILITY, SUPABASE_TABLES_BY_CAPABILITY, TABLES_BY_CAPABILITY, checkDatabase, missingRequiredTables, qualifiedBaseTableNames } from '../../../scripts/setup/checks/database.mjs';
...
function fakeDb(manifest: any = baseManifest, overrides: Record<string, unknown> = {}) {
  ...
  const tables = [...new Set([
    ...
    ...(manifest.database === 'supabase' ? Object.values(SUPABASE_TABLES_BY_CAPABILITY).flat() : []),
  ])];
    ...
    prepare(sql: string) {
      const statement = {
        ...
        async all() {
          ...
          if (sql === 'SELECT name FROM _migrations ORDER BY name') return rowResult((overrides.migrations as any) ?? SUPABASE_MIGRATIONS.map((name) => ({ name })));
...
describe('doctor database check', () => {
  ...
  it('probes every operational table owned by every capability plus always-required core tables', async () => {
    ...
    expect(TABLES_BY_CAPABILITY.portal).toEqual(expect.arrayContaining([
      'group_members', 'group_files', 'event_admins', 'prayer_items',
    ...
    const migrationFiles = [...SUPABASE_MIGRATIONS];
  ...
  it('requires private relations only for the corresponding Supabase capabilities', async () => {
    ...
    const publicOnly = relationRows([
      ...new Set([...ALWAYS_REQUIRED_TABLES, ...Object.values(TABLES_BY_CAPABILITY).flat(), ...Object.values(SUPABASE_TABLES_BY_CAPABILITY).flat()]),
  ...
  it('verifies Supabase migration completeness against qualified public and private relations', async () => {
    ...
    const publicTables = [...new Set([...ALWAYS_REQUIRED_TABLES, ...Object.values(TABLES_BY_CAPABILITY).flat(), ...Object.values(SUPABASE_TABLES_BY_CAPABILITY).flat()])];
