...
describe('wrangler replacement preflight', () => {
  ...
  it('requires a separate interactive replacement confirmation and emits a concise redacted diff', async () => {
    ...
    await expect(preflightWranglerConfig({ current, baseline, template, existingManifest: null, catalog: rawCatalog, interactive: true, forceConfig: false, output, confirmConfigReplacement }))
