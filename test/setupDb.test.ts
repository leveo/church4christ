...
describe('setup database operations on D1', () => {
  ...
  it('creates and reruns the same administrator idempotently', async () => {
    ...
    const row = await env.DB.prepare('SELECT display_name, lang FROM people WHERE email=?')
      .bind('admin@setup.test').first<{ display_name: string; lang: string }>();
    expect(row).toEqual({ display_name: 'Setup Admin', lang: 'en' });
  });
