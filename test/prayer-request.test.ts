...
// valid submit stores + returns 'sent', bad/missing fields return 'error' without
// storing, honeypot is silently accepted, and the Referer sanitizer keeps
...
describe('submitPrayerRequest', () => {
  it('stores a valid submission and returns sent', async () => {
    ...
    const outcome = await submitPrayerRequest(env.DB, form({ name: '王弟兄', email: 'wang@example.com', message: '请为我的家人祷告。' }));
  ...
  it('accepts a message-only submission (name + email optional)', async () => {
    const before = await count();
    expect(await submitPrayerRequest(env.DB, form({ message: 'Just a prayer, no contact info.' }))).toBe('sent');
    expect(await count()).toBe(before + 1);
  ...
  it('rejects an empty message and a malformed email without storing', async () => {
    const before = await count();
    expect(await submitPrayerRequest(env.DB, form({ name: 'x', email: 'a@b.c', message: '' }))).toBe('error');
    expect(await submitPrayerRequest(env.DB, form({ name: 'x', email: 'not-an-email', message: 'hi' }))).toBe('error');
    expect(await count()).toBe(before);
  });

  it('caps the stored message at 4000 characters', async () => {
    await submitPrayerRequest(env.DB, form({ message: 'a'.repeat(5000) }));
